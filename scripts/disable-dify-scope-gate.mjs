...
for (const line of rows) {
  ...
  for (const node of graph.nodes ?? []) {
    if (node.id !== "check_relevance" || node.data?.type !== "code") continue;
    node.data.code = ALWAYS_RELEVANT_CODE;
    node.data.desc = "已关闭范围拦截，车型导购问题统一进入产品检索与LLM回答。";
    disabledNodes += 1;
    changed = true;
  }
...
const backupName = `dify-workflows-before-disable-scope-gate-${timestamp}.json`;
...
console.log(`disabledScopeNodes=${disabledNodes}`);
